...
class HrEmployee(models.Model):
    ...
    techskill_ids = fields.One2many('emp.tech.skills','employee_id','Technical Skills')
    nontechskill_ids = fields.One2many('emp.nontech.skills','employee_id','Non-Technical Skills')
    education_ids = fields.One2many('employee.education','employee_id','Education')
    certification_ids = fields.One2many('employee.certification','employee_id','Certification')
    profession_ids = fields.One2many('employee.profession','employee_id','Professional Experience')
...
class TechTech(models.Model):
    ...
    _sql_constraints = [
        ('tech_unique', 'unique (name)', 'The name of the Technologies already exixts!'),
...
class EmployeeNonTechSkills(models.Model):
    ...
    applicant_id = fields.Many2one('hr.applicant','Applicant')
    employee_id = fields.Many2one('hr.employee','Employee')
    nontech_id = fields.Many2one('nontech.nontech','Non-Technical Skills', ondelete="cascade")
    levels = fields.Selection([('basic','Basic'),
                               ('medium','Medium'),
                               ('advance','Advance')],'Levels')
...
class NontechNontech(models.Model):
    ...
    _sql_constraints = [
        ('nontech_unique', 'unique (name)', 'The name of the Non Technical skills must be unique!'),
        ('seq_uniq', 'unique (sequence)', "Sequence name already exists!")
    ]
    ...
    def unlink(self):
        ...
        @param self : object pointer
        """
        tech_skill = self.env['emp.nontech.skills'].search([('nontech_id','in',self.ids)])
        if tech_skill:
            raise UserError(_('You are trying to delete a Skill which is referenced by an Employee.'))
...
class CertCert(models.Model):
    ...
    _sql_constraints = [
        ('cert_unique', 'unique (name)', 'The name of the Certifications must be unique!'),
        ('seq_uniq', 'unique (sequence)', "Sequence name already exists!")
    ]
...
class HrApplicant(models.Model):
    ...
    techskill_ids = fields.One2many('emp.tech.skills','applicant_id','Technical Skills')
    nontechskill_ids = fields.One2many('emp.nontech.skills','applicant_id','Non-Technical Skills')
    education_ids = fields.One2many('employee.education','applicant_id','Education')
    certification_ids = fields.One2many('employee.certification','applicant_id','Certification')
    profession_ids = fields.One2many('employee.profession','applicant_id','Professional Experience')
