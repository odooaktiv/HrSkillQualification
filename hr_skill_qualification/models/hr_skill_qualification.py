    ...
    def check_from_date(self):
        ...
        if (self.from_date > date) or (self.to_date > date):
            raise ValidationError(
                'Future Start Date or End Date in \
                Professional experience is not acceptable!!')
            ...
            if applicant.job_id and (applicant.partner_name or contact_name):
                ...
                employee = self.env['hr.employee'].create(
                    {'name': applicant.partner_name or contact_name,
                        ...
                        'nontechskill_ids':
                            [(6, 0, self.nontechskill_ids.ids)],
                        'education_ids':
                            [(6, 0, self.education_ids.ids)],
                        'certification_ids':
                ...
                applicant.job_id.message_post(
                    body=_(
                        'New Employee %s Hired') % applicant.partner_name
                    if applicant.partner_name else applicant.name,
                    subtype="hr_recruitment.mt_job_applicant_hired")
                employee._broadcast_welcome()
            else:
                raise UserError(
                    _('You must define an Applied Job \
                        and a Contact Name for this applicant.'))
